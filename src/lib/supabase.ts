...
export const configService = {
  async get() {
    ...
    return {
      ...
      defaultLlm: (data.default_llm || 'groq') as import('./supabase-store').LlmProvider,
      defaultModel: data.default_model || 'llama-3.1-8b-instant',
  ...
  async update(config: any) {
    const row: any = {
      ...
      default_llm: config.defaultLlm || 'groq',
      default_model: config.defaultModel || 'llama-3.1-8b-instant',
